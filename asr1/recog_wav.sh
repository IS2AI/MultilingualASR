...
if [ ! -f path.sh ] || [ ! -f cmd.sh ]; then
    echo "Please change current directory to recipe directory e.g., egs/asr1_mult/asr1"
    exit 1
fi
...
# rnnlm related
use_lang_model=true
lang_model=/rnnlm.model.best #if use_lang_model=true, path to rnnlm.model.best

# decoding parameter
cmvn= #path to cmvn.ark for example data/train/cmvn.ark
recog_model=results/model.last10.avg.best  #path to model.last10.avg.best 
